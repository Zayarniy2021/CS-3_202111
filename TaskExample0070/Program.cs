...
class DemoTask
{
    ...
    static void Main()
    {
        ...
        //Второй пример
        Task<double> tsk2 = Task<double>.Factory.StartNew(SumIt, 3);//Создаем задачу, которая возвращает int

        //Task<int> tsk3 = new Task<int>(SumIt, 3);
        Task<double> tsk3 = new Task<double>(new Func<object, double>(SumIt), 3);
        tsk3.Start();
